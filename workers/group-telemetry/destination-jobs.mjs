...
export async function checkDestinationMetadata({
  ...
  const { jobs = [] } = await control.send("destination_claim", { limit: 1 }, { requirePayload: true });
  const job = jobs[0];
  if (!job) return 0;
  const lease = { key: job.key, leaseToken: job.leaseToken };
  const release = () => control.send("destination_release", lease).catch(() => undefined);
  let result;
  try {
    await heartbeat();
    ...
    if (error?.category === "rate_limit") {
      ...
      // Keep the lease held unless the account-wide cooldown is acknowledged.
      // A sibling must not reclaim work during an unrecorded provider backoff.
      const cooldown = await control.send("proof_rate_limit", { retryAfterMs, now: clock() }).catch(() => null);
      if (cooldown?.recorded === true) await release();
