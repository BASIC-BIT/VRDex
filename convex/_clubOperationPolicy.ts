...
export function assessClubOperation(authority: OperationAuthority, operation: ClubOperation): OperationAssessment {
  ...
  if ((operation.kind === "assign_role" || operation.kind === "remove_role") && authority.actorKind !== "owner" && !authority.permittedProviderRoleIds.some(id => id.toLowerCase() === operation.roleId.toLowerCase())) return deny("role_not_assignable");
