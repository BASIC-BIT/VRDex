...
Community-submitted public profiles now start at `/submit`. Person, community, world, and event pages all render from the site root at `/<slug>` — one global slug namespace, so a slug has exactly one owner.
