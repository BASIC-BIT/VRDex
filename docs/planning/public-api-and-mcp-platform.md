...
- Run the implementation-time major MCP client smoke matrix against a deployed preview or production-like environment, including anonymous hosted reads, OAuth through Dynamic Client Registration, OAuth through public-client Client ID Metadata Documents, and local stdio configuration. Track results in `docs/developers/mcp-client-smoke-results.json` and run `VRDEX_MCP_CLIENT_MATRIX_REQUIRE_READY=1 pnpm check:mcp-client-matrix` before external readiness.
