...
September 23 Task 8 local follow-up: suggestion confirmation and rejection now recheck current recap access. Independent minute-based bounded scans expire unsent work beyond its current 15-minute grace even without a collector, including preexisting rows. Private membership day and range charts use compact poll coverage and break across explicit or silent outages; legacy windows retain exact observations without inferred continuity. Mounted current-day range continuity now expires against server-calibrated monotonic time even without another collector write. The Task 8 report records scoped backend/model checks, desktop/mobile segmentation and screenshots, types, lint and documentation checks. Hosted deployment, provider behavior and exact-head remote readiness remain unverified.

September 24 Task 9 local follow-up: long preflight backoff settles missed actions without truncating the provider's retry window. Batch cancellation reuses one current actor while preserving per-job authorization and atomic rollback. Telemetry Live and direct detail now project separate six-minute session freshness; complete chronological Instance history and Home Recent instances retain stale open and confirmed closed sessions. Mounted expiry removes current claims and closure confirmations without changing stored history. A category-authorized clock query calibrates each list owner once, preserving empty-page continuation and later observations without restarting pagination. Provider management consumes its existing independent 60-second freshness flag. The Task 9 handoff records backend and worker checks plus desktop/mobile actual-component expiry, recovery and retained-history evidence. No hosted transaction-limit failure, live provider behavior or deployment is claimed.

September 24 review follow-up: the current group-member card shows the observation time of its retained count, which can remain unchanged through the six-hour heartbeat interval. The Live instance query indexes open, recently observed sessions so newer closed rows cannot hide a long-running live instance. Staff without `manage_events` see only published events in the shared picker; event recap pagination fills visible pages past draft rollups within a bounded scan.
...
| Fresh telemetry Live and complete Instance history, peak/time-weighted average | Separate session freshness and chronological retained summaries implemented locally | Backend freshness/history pagination and actual-component expiry checks; hosted proof remains |
...
Audit collection resumes one persisted provider page per leased telemetry poll, reserves an additional request, and retains raw provider offsets when boundary rows are filtered. It uses fixed windows, a one-minute ingestion delay and five-minute overlap on completed windows. Each window now requires two identical full raw audit-ID passes followed by staged-page finalization before coverage is marked complete. A changed page sequence restarts the scan; rows from discarded passes remain unverified and hidden from movement/activity reads. Late provider events after verification remain a provider-reconciliation limitation, not proof of exhaustive historical capture. Worker tests cover authority budgets, audit normalization and collection failure/boundary behavior. Provider errors never certify a scan as complete. No live provider data was fetched by these tests.

Feature-disable integration review added transactional guards against telemetry/audit ingestion after analytics is disabled. The worker skips those reads while continuing readiness refreshes. Public and staff current-population summaries stop presenting current values immediately; historical rows remain retained. Focused telemetry, membership and analytics regressions pass. `view_members` is an explicit staff capability, included in newly initialized Admin/Moderator presets; existing saved roles are not silently expanded. Staff with role assignment or member removal permission can use a direct VRChat user ID action form, including when they also have directory access but the provider cannot serve the directory read. Members tabs follow live permission changes, clearing stale selections and reviews. Unauthorized direct Members visits show the access notice without requesting provider context.
