...
Implementation evidence: unauthenticated Discord invite lookup returned the public VRChat server name/icon. A public VRChat image exercised the real signed redirect and produced a sanitized static 128px WebP. Controlled responses cover names, privacy, failures, and destination replacement. Desktop and 390px mobile previews were visually inspected, including long names and unavailable artwork. The 2026-09-08 authenticated BASICBIT response confirmed a distinct profile-picture override thumbnail and avatar. Its `/api/1/image/file_<UUID>/<version>/512` endpoint redirects to a signed thumbnail CDN URL. The rollout correction produced a static 128px WebP (2,648 bytes) through the real importer. BASIC approved host-based artwork validation on 2026-09-08: accept provider-returned HTTPS URLs on exact trusted provider hosts without prescribing paths, thumbnail sizes, or signature formats. Every redirect still rechecks the host and public DNS/IP boundary; byte, timeout, decode, and re-encoding limits remain. Regression tests cover override precedence, changing URL formats, untrusted redirects, and private DNS.
