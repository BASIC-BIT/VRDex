...
4. **Dispatch `Staging Deploy` with `clerk_instance_rotation` enabled.** A plain re-run aborts by design: the pre-deploy comparison measures the new issuer against the key the *currently deployed* app serves, which is the old instance until Vercel publishes. The input skips only that comparison — the issuer format check and the post-deploy comparison still run, so the new pair must agree by the end of the run.
