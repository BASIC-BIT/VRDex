...
Any authenticated provider 401 immediately sets the account and every assigned integration to `auth_required`, releases all leases, and opens degraded coverage. Recover by creating a new session from behind the collector's NAT gateway, never from an operator's own network: VRChat pins a session to the address that created it, and a laptop-born session is refused on the collector's first request. The login host stack in `infra/terraform/group-telemetry-login-host` (its README is the runbook) provides the tunnel; run `pnpm proof:group-telemetry --fresh-login` and then `pnpm ops:vrchat-session:transfer` (`scripts/transfer-vrchat-session-to-aws.mjs`) through it. The transfer validates the session, replaces only the session fields, and mints a fresh worker key; do not manually extract the local vault record. Re-register the account with the printed `workerKeyHash` to increment credential generation, restart the task, then return the account to `ready`. Password-based unattended reauthentication is intentionally absent. Recovery puts integrations into `connecting`; it does not backfill the outage or turn it into zero attendance.
...
| Reading | Meaning |
| --- | --- |
| `state: auth_required`, `lastHealthResult: provider_401` | VRChat rejected the collector's session. It may have expired or been created from a different network address; a successful check from a laptop does not prove it works from the collector. The control plane answers the worker's heartbeat with 423 `collector_disabled`, the worker fails six times and exits 1, and ECS restarts it about every five minutes. Those stopped tasks and `collector_worker_restart` log events are the expected shape of this state, not a second failure. Recover with the NAT-origin login path above. |
...
The worker checks its session at startup and then at jittered 8-12 minute intervals, including when no telemetry integrations or proofs are queued. A release readiness check that requires `vrchat_proof_v1` fails while the account is `auth_required`; that failure is correct and clears once the session is replaced and the account is back to `ready`.
