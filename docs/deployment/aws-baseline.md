...
so the SES stack was never rewired into a sign-in flow.
...
[`ses-auth-email.md`](./ses-auth-email.md).
...
- `VRDEX_SUPPORT_DIGEST_TO` the mailbox the support digest is delivered to. Without it the digest is switched off and requests accumulate with nobody notified
