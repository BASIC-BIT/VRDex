...
- Every valid VRCDN link has one permanent profile Watch block, regardless of liveness or provenance. It contains `Open preview`, `Quest (MPEG-TS)`, and `PC (RTSPT)`. These controls do not move when the player appears or disappears, and VRCDN is not duplicated as a generic Links button. Community-submitted streams get the controls but cannot claim `Live now` until the link is owner-authored, reviewed, or partner-provided. `Open preview` points at `panel.vrcdn.live/preview/<id>`, which `#217` had listed as an operator surface not to publish; `#296` reversed that as a locked decision because the controls are useful whether or not the stream is live.
