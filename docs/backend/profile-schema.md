...
- **`ended` is playback termination, not stream liveness.** A clean EOF also arrives when the CDN closes or recycles a connection while the broadcaster is still going, so the terminal state always offers a retry rather than stranding a viewer on a stream that never stopped. Retry returns to the poster, so reconnecting still costs a viewer slot only on request.
