...
Provider liveness is not stored on the profile. Twitch is read during the server render from `apps/web/src/lib/server/twitch-live.ts`. VRCDN is read after the profile renders through the profile-scoped `/api/profile-live/[slug]/vrcdn` route and `apps/web/src/lib/server/vrcdn-live.ts`, so a slow media-server answer cannot hold the page. Both providers cache successful observations for sixty seconds and render as a `Live now` badge in the `Watch` surface. An unavailable VRCDN answer receives one bounded retry; an authoritative offline answer does not. The VRCDN block stays hidden until a live answer arrives, and the first unavailable answer cannot replace a previously confirmed live state while its retry is pending. This keeps the complete block together and prevents a transient probe failure from unmounting an active player. If the final retry cannot produce a valid answer, blocks with explicit per-stream unavailable evidence pending that retry are removed rather than retaining an unconfirmed `Live now` claim; other confirmed streams remain visible. A failure of the profile-live route itself provides no per-stream evidence and does not clear a server-rendered confirmed state. Community-submitted VRCDN links remain ordinary profile links because their provenance cannot support a liveness claim.
