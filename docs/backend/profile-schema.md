...
Provider liveness is not stored on the profile. Twitch and VRCDN are read during the server render from `apps/web/src/lib/server/twitch-live.ts` and `apps/web/src/lib/server/vrcdn-live.ts`. VRCDN is then revalidated after hydration through the profile-scoped `/api/profile-live/[slug]/vrcdn` route. This puts the bounded initial provider check on the profile render path so a confirmed live player is present in the first visible render instead of disappearing and returning across a refresh. Both providers cache successful observations for sixty seconds and render as a `Live now` badge in the `Watch` surface. An unavailable VRCDN answer receives one bounded client retry; an authoritative offline answer does not. The first unavailable answer cannot replace a previously confirmed live state while its retry is pending. This keeps the complete block together and prevents a transient probe failure from unmounting an active player. If the final retry cannot produce a valid answer, blocks with explicit per-stream unavailable evidence pending that retry are removed rather than retaining an unconfirmed `Live now` claim; other confirmed streams remain visible. A failure of the profile-live route itself provides no per-stream evidence and does not clear a server-rendered confirmed state. Community-submitted VRCDN links remain ordinary profile links because their provenance cannot support a liveness claim.
...
- Only `owner_authored`, `reviewed`, and `partner_provided` links carry a live claim, and this applies to **both providers**. `submitCommunityProfile` publishes immediately and a community submission is one signed-in person adding somebody else's profile, so a `community_submitted` channel or stream id could belong to anyone; the link still renders, but it makes no claim about who is streaming.
...
- Nothing sweeps on a schedule. A stream nobody opens is never probed. A viewed profile probes during its server render and revalidates after hydration, with one client retry only when the first client answer is unavailable; successful observations remain shared for sixty seconds.
