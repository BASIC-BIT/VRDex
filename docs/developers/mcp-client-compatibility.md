...
https://staging.vrdex.net/mcp` accepted all generated local-stdio,
...
| Codex desktop and CLI | Uses project-scoped `.codex/config.toml` or `codex mcp add`. | Uses Streamable HTTP at the canonical hosted `/mcp` endpoint. | Supports protected-resource discovery, DCR or CIMD, PKCE, and `codex mcp login`; request only the scopes needed by the enabled tools. | Codex desktop package 26.825.6671.0 passed a production OAuth contribution flow on 2026-08-31, including `vrdex_profile_submit` and `vrdex_profile_update`, without recording profile identifiers. Local stdio and hosted anonymous real-client smokes remain pending. Codex CLI 0.145.0 previously passed staged OAuth initialization and tools/list for the event-write configuration. On 2026-09-01 its explicit production login with expanded profile/media scopes succeeded, but the following isolated startup exposed no VRDex tools; the production CLI startup row remains pending without treating it as negative server evidence. |
...
0. Run `pnpm smoke:mcp-compat`; for hosted protocol coverage, add
   ...
   PR #159's 2026-07-09 repository audit shows that OAuth subcheck remains
...
session configured with a bearer credential lists eleven tools: those six reads,
`vrdex_list_my_profiles`, and `vrdex_event_create`, `vrdex_event_update`,
`vrdex_profile_update`, and `vrdex_profile_submit`. Registration does not inspect
the token's scopes, so all five credentialed tools appear whenever a credential
is present regardless of what that credential may actually read or write.

1. Claude Desktop local stdio starts, lists six anonymous tools or eleven
...
4. Gemini CLI local stdio lists six anonymous tools or eleven credentialed
   ...
   OAuth fallback.
5. VS Code local stdio lists six anonymous tools or eleven credentialed tools
