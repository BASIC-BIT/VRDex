...
async function writeCredentialFiles(args: {
  ...
  await writeFile(
    ...
    [
      ...
      "```powershell",
      `. ${envPs1}`,
      `corepack pnpm smoke:mcp-claude-code -- --mode hosted-http --hosted-url ${hostedUrl} --hosted-data`,
      `corepack pnpm smoke:mcp-inspector -- --hosted-url ${hostedUrl} --hosted-data`,
      "```",
      "",
      "Bash:",
      "",
      ...
      "```",
      "",
      "After both smokes pass, record sanitized evidence with `pnpm record:mcp-client-smoke` for `claude-code/hosted-oauth` and `mcp-inspector/hosted-oauth`.",
      "",
    ].join("\n"),
  );
