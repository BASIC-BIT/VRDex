...
function manualEvidenceTemplates(options: Options): EvidenceTemplate[] {
  ...
  return [
    ...
    },
    {
      ...
      environment: `Windows / Claude Code / ${hostedUrl}`,
      hosted: true,
      ...
      setup: [
        ...
        "# Or use claude mcp add --transport http vrdex against the same hosted target and capture sanitized client output.",
      ].join("\n"),
      targetEnvironment,
    },
    {
    ...
    },
    {
      ...
      environment: `Windows / MCP Inspector CLI / ${hostedUrl}`,
      hosted: true,
      ...
      setup: `pnpm smoke:mcp-inspector -- --hosted-url ${hostedUrl} ${inspectorHostedDataArgs(options)}`,
      targetEnvironment,
    },
    {
      check: "hosted-oauth",
