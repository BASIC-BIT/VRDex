...
function LineChart({
  ...
  const drawablePoints = points.filter(
    (point) =>
      ...
      point.state === "estimated",
  );
  if (drawablePoints.length < 2)
    return (
      <Notice variant="dashed">Not enough observed data for this range.</Notice>
    );
  const width = 720;
  const height = 220;
  ...
  for (const point of drawablePoints) {
    ...
    if (!segment || !previous || point.at - previous.at > maxGapMs)
      segments.push([point]);
  ...
  return (
    <div>
      <svg
        aria-label={`${label}. Maximum ${formatNumber(maxValue)}.`}
      ...
      >
        ...
        {segments.map((segment, index) => (
          <polyline
            ...
            strokeWidth="3"
            strokeDasharray={
              (segment.at(-1)?.state ?? segment[0]?.state) === "estimated"
                ? "8 7"
                : undefined
            }
          />
...
function CommunityTelemetryDashboardContent({
  ...
  const reviewAssociationSuggestion = useMutation(
    api.communityTelemetry.reviewAssociationSuggestion,
  );
  ...
  const referenceNow =
    fixtureData?.integration.lastSuccessfulObservationAt ?? Date.now();
  const cutoff = referenceNow - rangeHours * 60 * 60_000;
  const population = useMemo(
    () =>
      (data?.population ?? []).filter((point) => point.observedAt >= cutoff),
    [data, cutoff],
  );
  const members = useMemo(
    () =>
      (data?.memberCounts ?? []).filter((point) => point.observedAt >= cutoff),
    [data, cutoff],
  );
  const populationPoints = useMemo(() => {
    if (!data) return [];
    if (rangeHours === 24)
      return population.map((point) => ({
        at: point.observedAt,
        value: point.totalPopulation,
        state: point.coverageState,
      }));
  ...
  const activeInstancePoints = useMemo(() => {
    ...
    if (rangeHours === 24)
      return population.map((point) => ({
        at: point.observedAt,
        value: point.activeInstanceCount,
        state: point.coverageState,
      }));
    const grain = rangeHours === 168 ? "hour" : "day";
    return data.rollups
  ...
  if (!mounted || isAuthLoading || data === undefined)
    return <Notice>Loading telemetry…</Notice>;
  if (data === null)
    return (
      <div className="grid gap-5">
        ...
        {canManageIntegrations ? (
          <Link
            ...
            href={`/account/communities/${encodeURIComponent(communitySlug)}/connection`}
          >
            Group connection
          </Link>
        ) : null}
  ...
  const canRead = (category: string) =>
    data.readableCategories?.includes(category) ?? true;

  const suggested = data.associations.filter(
    (association) => association.state === "suggested",
  );
  const eventNames = new Map(
    data.events.map((event) => [event._id, event.title]),
  );
  const eventRecaps = data.rollups
    .filter((rollup) => rollup.grain === "event")
    .slice(-6)
    .reverse();
  const recentSessions = data.sessions.slice(0, 8);
  const inspectedSession =
    recentSessions.find((session) => session._id === inspectedSessionId) ??
    recentSessions[0];
  const inspectedInstancePoints = inspectedSession
    ? data.instancePopulation
        .filter(
          (point) =>
            point.sessionId === inspectedSession._id &&
            point.observedAt >= cutoff,
        )
        .map((point) => ({
          at: point.observedAt,
  ...
  return (
    <div className="grid gap-8 ph-no-capture" data-ph-no-capture>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="mt-1 text-4xl font-semibold tracking-tight">Home</h1>
          <p className="mt-3 text-sm text-muted">
            Last observed{" "}
            {formatTime(data.integration.lastSuccessfulObservationAt)}
          </p>
        </div>
      ...
      {data.integration.freshness === "stale" ? (
        <Notice variant="warning">
          Collection is stale. Missing time remains a gap and is not reported as
          zero attendance.
        </Notice>
      ) : null}
      ...
      {data.integration.state === "degraded" ? (
        <Notice variant="warning">
          Collection is retrying with backoff. Coverage remains marked as
          degraded until a successful poll.
        </Notice>
      ) : null}
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-5">
        {canRead("current_population") ? (
          <SummaryCard
            detail={`${data.summary.activeInstanceCount} active instances`}
            label="Current population"
            value={formatNumber(data.summary.currentPopulation)}
          />
        ) : null}
        {canRead("population_history") ? (
          <SummaryCard
            detail="Observed range"
            label="Peak concurrency"
            value={formatNumber(data.summary.peakConcurrency)}
          />
        ) : null}
        {canRead("population_history") ? (
          <SummaryCard
            detail="Gap-aware estimate"
            label="Player hours"
            value={formatNumber(data.summary.playerHours, 1)}
          />
        ) : null}
        {canRead("group_size") ? (
          <SummaryCard
            detail={
              canRead("membership_movement")
                ? `${formatNumber(data.summary.groupMemberGrowth)} growth`
                : ""
            }
            label="Group members"
            value={formatNumber(data.summary.groupMemberCount)}
          />
        ) : null}
        {canRead("population_history") ? (
          <SummaryCard
            detail="Observed time only"
            label="Coverage"
            value={`${formatNumber((data.summary.coverageRatio ?? 0) * 100, 1)}%`}
          />
        ) : null}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        {canRead("population_history") ? (
          <Card padding="lg">
            ...
            <div className="mt-7">
              <LineChart
                label="Population over time. Solid segments are observed, dashed segments are estimated, and missing coverage is blank"
                maxGapMs={
                  rangeHours === 24
                    ? 6 * 60_000
                    : rangeHours === 168
            ...
            <p className="mt-4 text-xs text-muted">
              Solid observed · dashed estimated · blank missing
            </p>
          </Card>
        ) : null}
        {canRead("population_history") ? (
          <Card padding="lg">
            ...
            <div className="mt-7">
              <LineChart
                label="Active instances over time"
                maxGapMs={
                  rangeHours === 24
                    ? 6 * 60_000
                    : rangeHours === 168
        ...
        {canRead("group_size") ? (
          <Card padding="lg">
            ...
            <div className="mt-7">
              <LineChart
                label="Group members over time"
                maxGapMs={7 * 60 * 60_000}
                points={members.map((point) => ({
                  at: point.observedAt,
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        {canRead("instance_history") ? (
          <Card padding="lg">
            <SectionHeading description="A session closes after consecutive missed observations. Reopened provider instances become a new session.">
              Recent instances
            </SectionHeading>
            <div className="mt-6 grid gap-2">
              {recentSessions.length === 0 ? (
                <Notice variant="dashed">
                  No instance sessions observed yet.
                </Notice>
              ) : (
                recentSessions.map((session) => {
                  ...
                  const durationMinutes = Math.max(
                    0,
                    Math.round((endedAt - session.openedAt) / 60_000),
                  );
                  return (
                    <div
                      className="grid gap-1 border-b border-border py-3 text-sm sm:grid-cols-[1fr_auto_auto] sm:gap-5"
                      key={session._id}
                    >
                      <div>
                        <span className="font-medium">
                          {session.vrchatWorldId}
                        </span>
            ...
            {inspectedSession ? (
              <div className="mt-7 border-t border-border pt-6">
                <Field>
                  ...
                  <Select
                    onChange={(event) =>
                      setInspectedSessionId(event.target.value)
                    }
                    value={inspectedSession._id}
                  >
                    {recentSessions.map((session) => (
                      <option key={session._id} value={session._id}>
                        {session.vrchatWorldId} · {formatTime(session.openedAt)}
                      </option>
                    ))}
                  </Select>
                </Field>
                <div className="mt-5">
                  <LineChart
                    label={`${inspectedSession.vrchatWorldId} instance population`}
                    points={inspectedInstancePoints}
                  />
                </div>
              </div>
            ) : null}
        ...
        {canRead("current_population") || canRead("event_recaps") ? (
          <Card padding="lg">
            {canRead("current_population") ? (
              <>
                <SectionHeading description="Current observed distribution, aggregated without user identities.">
                  Active worlds
                </SectionHeading>
                <div className="mt-6 grid gap-2">
                  {(data.summary.worlds ?? []).length === 0 ? (
                    <Notice variant="dashed">No active worlds observed.</Notice>
                  ) : (
                    (data.summary.worlds ?? []).map((world) => (
                      <div
                        className="grid grid-cols-[1fr_auto] gap-4 border-b border-border py-3 text-sm"
                        key={world.worldId}
                      >
                        <div>
                          <span className="font-medium">{world.worldId}</span>
                          <span className="mt-1 block text-xs text-muted">
                            {formatNumber(world.samples)} active{" "}
                            {world.samples === 1 ? "instance" : "instances"}
                          </span>
                        </div>
            ...
            {canRead("event_recaps") && eventRecaps.length > 0 ? (
              <div className="mt-8">
                <h3 className="text-xl font-semibold tracking-tight">
                  Recent event recaps
                </h3>
                <div className="mt-4 grid gap-3">
                  {eventRecaps.map((recap) => (
                    <div
                      className="border-t border-border pt-4 text-sm"
                      key={`${recap.eventId ?? "event"}-${recap.bucketStartAt}`}
                    >
                      <p className="font-medium">
                        {recap.eventId
                          ? (eventNames.get(recap.eventId) ?? "Event")
                          : "Event"}
                      </p>
      ...
      {canRead("population_history") ? (
        <div>
          <Card padding="lg">
            <SectionHeading description="Unknown and degraded windows remain visible instead of becoming zeroes.">
              Coverage
            </SectionHeading>
            <div className="mt-6 grid gap-2">
              {data.coverage.slice(-12).map((window) => (
                <div
                  className="grid gap-2 border-b border-border py-3 text-sm sm:grid-cols-[8rem_1fr_auto]"
                  key={`${window.startedAt}-${window.state}`}
                >
                  <span
                    className={cn(
                      "font-medium",
                  ...
                  <span>
                    ...
                    {window.endedAt ? formatTime(window.endedAt) : "now"}
                  </span>
                  <span className="text-muted">
                    {window.reason ?? "Provider observation"}
                  </span>
                </div>
              ))}
            </div>
              ...
              {suggested.map((association) => (
                ...
                >
                  <span className="text-sm">
                    Suggested match ·{" "}
                    {formatNumber(association.confidence * 100)}% confidence
                  </span>
                  <div className="flex gap-2">
                    <Button
                      disabled={busy || Boolean(fixtureData)}
                      onClick={() =>
                        perform(
                          () =>
                            reviewAssociationSuggestion({
                              communitySlug,
                              associationId:
                                association._id as Id<"eventInstanceAssociations">,
                              state: "confirmed",
                            }),
...
export function CommunityTelemetryAccessNotice({
  kind,
}: {
  kind: "forbidden" | "signed-out" | "unavailable";
}) {
  if (kind === "signed-out") {
    return (
      <Card padding="lg" surface="strong">
        ...
        <p className="mt-3 text-sm leading-7 text-muted">
          This private dashboard is available to authorized community staff.
        </p>
        <Link
          className={cn(
...
> {
  ...
  static getDerivedStateFromError(error: unknown) {
    return {
      error:
        error instanceof Error ? error : new Error("Telemetry query failed."),
    };
  }
  ...
  componentDidUpdate(previousProps: {
    ...
    if (
      previousProps.communitySlug !== this.props.communitySlug &&
      this.state.error
    ) {
  ...
  render() {
    if (this.state.error) {
      const forbidden = this.state.error.message.includes(
        "permission to manage this community integration",
      );
      return (
        <CommunityTelemetryAccessNotice
          kind={forbidden ? "forbidden" : "unavailable"}
        />
      );
    }
...
export function CommunityTelemetryDashboard({
  ...
  return (
    <CommunityTelemetryDashboardErrorBoundary communitySlug={communitySlug}>
      <CommunityTelemetryDashboardContent
