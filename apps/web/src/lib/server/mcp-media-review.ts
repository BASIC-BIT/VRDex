import { commandReceiptSchema, reviewDecisionSchema, reviewDetailSchema, z } from "@vrdex/api-contracts";
...
export const mediaReviewGetInputSchema = z.strictObject({ submissionId: mediaReviewSubmissionIdSchema });
...
export const mediaReviewWriteToolNames = ["vrdex_media_review_decide"] as const;
export const mediaSubmissionWriteToolNames = ["vrdex_media_submission_withdraw"] as const;

type ReviewQueryName = "list" | "detail" | "candidate";
type ReviewMutationName = "decide" | "withdraw";
type StoredObject = { body: Uint8Array; contentType: string; contentLength?: number };
...
export type MediaReviewDependencies<TActor extends string = string> = {
  ...
  query: (name: ReviewQueryName, args: Record<string, unknown>) => Promise<unknown>;
  mutate: (name: ReviewMutationName, args: Record<string, unknown>) => Promise<unknown>;
...
async function attestation<TActor extends string>(dependencies: MediaReviewDependencies<TActor>) {
  const emailVerified = await dependencies.verifyContributorEmail(dependencies.actorUserId)
    .catch(() => false);
  return {
...
async function rasterPreview(body: Uint8Array) {
  return await sharp(body, { limitInputPixels: 16_777_216, sequentialRead: true })
    .rotate()
...
export function createMcpMediaReviewHandlers<TActor extends string>(dependencies: MediaReviewDependencies<TActor>) {
  return {
    ...
    async get(input: unknown): Promise<ToolResult> {
      ...
      const result = await dependencies.query("detail", { ...(await attestation(dependencies)), ...value });
      if (result === null) return refusal("The media submission is unavailable for review.");
    ...
    async preview(input: unknown): Promise<ToolResult> {
      ...
      const before = reviewDetailSchema.nullable().parse(await dependencies.query("detail", args));
      ...
      if (before.reviewVersion !== value.expectedReviewVersion) {
        return refusal("The review changed. Inspect the current detail before requesting a preview.");
      }
      const descriptor = z.strictObject({
        ...
        profileDisplayName: z.string().max(1000),
      }).nullable().parse(await dependencies.query("candidate", args));
      if (descriptor === null) return refusal("The candidate is unavailable for review.");
      ...
      if (
        ...
        || stored.contentLength !== undefined && stored.contentLength !== stored.body.byteLength
        || stored.contentType !== descriptor.mimeType
        || before.candidate.contentSha256 === null
        || createHash("sha256").update(stored.body).digest("hex") !== before.candidate.contentSha256
      ) {
        return refusal("The stored candidate no longer matches the inspected review detail.");
      }
      const after = reviewDetailSchema.nullable().parse(await dependencies.query("detail", args));
      if (after === null || after.reviewVersion !== before.reviewVersion || after.candidate.rendition === null) {
        return refusal("The review changed while the preview was prepared. Inspect it again.");
      }
      let preview: Buffer;
      try {
      ...
      if (preview.byteLength > MAX_PREVIEW_BYTES) return refusal("The rendered preview is too large to return.");
      ...
      return {
        content: [{ type: "image", data: preview.toString("base64"), mimeType: "image/png" }],
        structuredContent,
      };
    },
    ...
    async decide(input: unknown): Promise<ToolResult> {
      ...
      const receipt = commandReceiptSchema.parse(await dependencies.mutate("decide", {
        ...(await attestation(dependencies)),
        ...value,
      }));
    ...
    async withdraw(input: unknown): Promise<ToolResult> {
      ...
      const withdrawn = z.boolean().parse(await dependencies.mutate("withdraw", {
        actorUserId: dependencies.actorUserId,
        submissionId: value.submissionId,
      }));
