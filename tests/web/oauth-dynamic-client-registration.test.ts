...
describe("OAuth dynamic client registration", () => {
  ...
  it("rejects invalid dynamic client metadata before registering a client", async () => {
    ...
    assert.deepEqual(await response.json(), {
      ...
      error_description: "Dynamic MCP clients can only request public:read mcp:read profile:read assets:review:read assets:contribute mcp:write assets:write assets:review:write assets:publish events:write profile:write profile:contribute.",
    });
  });
  ...
  it("refuses a transport write scope with no resource to write", async () => {
    ...
    assert.deepEqual(await response.json(), {
      error: "invalid_client_metadata",
      error_description:
        "Dynamic MCP write clients must request mcp:write and at least one of assets:write, assets:review:write, assets:publish, assets:contribute, events:write, profile:write, profile:contribute.",
    });
  });
